...
def Temporal_mean(data_short_period, dates_short_period, t_freq_short, t_freq_long, min_data_percentage) :
    ...
    if ( data_short_period.shape[0] != dates_short_period.shape[0] ) :
        ...
        # Step 2 : Calculate the mean over the longer time period.

        contiguous_data    = []
        contiguous_dates   = []
        total_rows         = data_short_period.shape[0]
        initial_time       = dates_short_period[0]       # time at which the averaging period begins; we begin counting at the first row
        ...
        # Step 3 : Return the results from the temporal averaging.
